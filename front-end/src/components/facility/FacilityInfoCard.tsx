...
const FacilityInfoCard = ({ facilityType, occupantCount }: FacilityInfoCardProps) => {
  const getFacilityDescription = (type: string) => {
    switch (type) {
      case 'office':
        return 'Office building with multiple departments';
      case 'warehouse':
        return 'Storage and distribution facility';
      default:
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">{getFacilityDescription(facilityType)}</p>
        </div>
